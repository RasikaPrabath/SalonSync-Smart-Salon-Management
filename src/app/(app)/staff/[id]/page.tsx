...
import { DEMO_STAFF, DEMO_SALES, DEMO_APPOINTMENTS } from '@/lib/demo-data'
...
export default async function StaffProfilePage({
  ...
  const staffAppointments = DEMO_APPOINTMENTS.filter(a => a.staffId === staff.id)
  const totalRevenue = staffAppointments.reduce((sum, a) => sum + a.price, 0)
      ...
      <Card>
        <CardContent className="pt-5">
          ...
          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-[hsl(var(--border-subtle))]">
            ...
            <div className="text-center">
              <p className="text-lg font-bold text-[hsl(var(--foreground))]">{staff.commissionPercent}%</p>
            ...
            {staffAppointments.map(apt => (
              <div key={apt.id} className="flex items-center justify-between py-3 border-b border-[hsl(var(--border-subtle))] last:border-0">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-[hsl(var(--foreground-muted))]">
                    +{formatCurrency((apt.price * staff.commissionPercent) / 100)} comm.
                  </p>
                </div>
              </div>
            ))}
