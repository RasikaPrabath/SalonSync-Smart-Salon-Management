...
export function ReportsClient({ sales, expenses }: { sales: any[], expenses: any[] }) {
  const [range, setRange] = useState<Range>('30d')

  const days = range === '7d' ? 7 : range === '30d' ? 30 : 90
  
  // 1. Build Daily Data for the charts
  const dailyData = []
  for (let i = days - 1; i >= 0; i--) {
    ...
    const dailyRevenue = sales
      .filter(s => new Date(s.date).toDateString() === compareDate)
      .reduce((sum, s) => sum + Number(s.amount), 0)

    const dailyExpenses = expenses
      .filter(e => new Date(e.date).toDateString() === compareDate)
      .reduce((sum, e) => sum + Number(e.amount), 0)

    dailyData.push({ date: dateStr, revenue: dailyRevenue, expenses: dailyExpenses, profit: dailyRevenue - dailyExpenses })
  }
  ...
  // Filter raw arrays for the selected range (for pie/bar charts)
  const cutoffDate = new Date()
  cutoffDate.setDate(cutoffDate.getDate() - days)
  const recentSales = sales.filter(s => new Date(s.date) >= cutoffDate)
  const recentExpenses = expenses.filter(e => new Date(e.date) >= cutoffDate)

  // 3. Expense Breakdown by Category
  const expenseMap = new Map<string, number>()
  recentExpenses.forEach(e => {
    // Parse category from description format: "[Category] Note" or use explicit category column
    let cat = e.category || 'Other'
    if (!e.category && e.description?.startsWith('[')) {
      cat = e.description.substring(1, e.description.indexOf(']'))
    }
    expenseMap.set(cat, (expenseMap.get(cat) || 0) + Number(e.amount))
  })
  ...
  recentSales.forEach(s => {
    const srv = s.service || 'Other'
    serviceMap.set(srv, (serviceMap.get(srv) || 0) + Number(s.amount))
  })
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-44">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyData.slice(-14)} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                <XAxis dataKey="date" tick={{ fontSize: 9, fill: 'hsl(var(--foreground-subtle))' }}
                  tickLine={false} axisLine={false} />
                <YAxis tick={{ fontSize: 9, fill: 'hsl(var(--foreground-subtle))' }} tickLine={false} axisLine={false}
        ...
        <Card>
          ...
          <CardContent>
            {expenseByCategory.length > 0 ? (
              ...
              <div className="h-44 flex items-center justify-center text-[hsl(var(--foreground-muted))]">
